...
public class ValidationDocumentGenerator(string? outputDirectory) : Generator(outputDirectory)
{
    public override void Generate(Schema schema)
    {
        var filename = Path.Join(OutputDirectory, "TestGenerateValidationDocument.cs");
            ...
            using (writer.OpenScope())
            {
                ...
                using (writer.OpenScope())
                {
                    ...
                    using (writer.OpenScope())
                    {
                        ...
                        {
                            string propertyName = GetPropertyName(property, out bool isExtension);
                            ...
                            using (writer.OpenScope())
                            {
                                ...
                                if (properties.FirstOrDefault(p => p.IsValue) is { } firstValueProperty)
                                {
                                ...
                                else
                                {
                                    foreach (var subProperty in properties.Where(p => !p.IsValue && !ExcludeSubPropertyFromValidation(p.Name)))
                                    {
                                        ...
                                        using (writer.OpenScope())
                                        {
                                            ...
                                            else if (subProperty.ValueType.AdditionalProperties is { } additionalProperties)
                                            {
                ...
                using (writer.OpenScope())
                {
                    ...
                    if (propertiesProperty != null && additionalProperties != null)
                    {
                        writer.WriteLine("using (var packet = m_writer.OpenPacket(m_output))");
                        using (writer.OpenScope())
                        {
                ...
                using (writer.OpenScope())
                {
                    ...
                    for (int i = 1; i <= 3; ++i)
                    {
                        ...
                        using (writer.OpenScope())
                        {
                            ...
                            {
                                var properties = property.ValueType.Properties;
                                var firstValueProperty = properties.First(p => p.IsValue);
                                ...
                                else
                                {
                                    foreach (var subProperty in properties.Where(p => !p.IsValue && !ExcludeSubPropertyFromValidation(p.Name)))
                                    {
                                        ...
                                        else if (properties.Any(p => p.ValueType.Name == "ReferenceList"))
                                        {
                                            ...
                                            {
                                                targetId = "Position";
                                                referencePropertyNames = ["position"];
                                            }
                                            else if (subProperty.ValueType.Name == "DoubleList")
                                            {
                                                targetId = "Double";
                                                referencePropertyNames = ["billboard", "scale"];
                                            }
                                            string valueSuffix = "";
                                            ...
                                            using (writer.OpenScope())
                                            {
                                                writer.WriteLine("w2.WriteReferences(CreateList(new Reference(\"Constant{0}1\", CreateList({1})), new Reference(\"Constant{0}2\", CreateList({1}))));", targetId, referencePropertyNames.Select(n => $"\"{n}\"").Join(", "));
                                                WriteAssertion(writer, isExtension, "expect(e.{0}.{1}.getValue(date){4}).toEqual([dataSource.entities.getById('Constant{2}1').{3}.getValue(date), dataSource.entities.getById('Constant{2}2').{3}.getValue(date)]);", propertyName, subPropertyName, targetId, referencePropertyNames.Join("."), valueSuffix);
                                            }
                                        }
                                        else if (properties.Any(p => p.ValueType.Name == "ReferenceListOfLists"))
                                        {
                                            ...
                                            using (writer.OpenScope())
                                            {
                                                writer.WriteLine("w2.WriteReferences(CreateList(CreateList(new Reference(\"Constant{0}1\", CreateList({1})), new Reference(\"Constant{0}2\", CreateList({1})), new Reference(\"Constant{0}3\", CreateList({1})))));", targetId, referencePropertyNames.Select(n => $"\"{n}\"").Join(", "));
                                                WriteAssertion(writer, isExtension, "expect(e.{0}.{1}.getValue(date){4}).toEqual([ new PolygonHierarchy([ dataSource.entities.getById('Constant{2}1').{3}.getValue(date), dataSource.entities.getById('Constant{2}2').{3}.getValue(date), dataSource.entities.getById('Constant{2}3').{3}.getValue(date) ])]);", propertyName, subPropertyName, targetId, referencePropertyNames.Join("."), valueSuffix);
                                            }
                                        }
                                        else if (subProperty.ValueType.Name.Contains("Material"))
                                        {
                            ...
                            if (properties.Any(p => p.Name == "velocityReference"))
                            {
                                ...
                                using (writer.OpenScope())
                                {
                                    ...
                                    using (writer.OpenScope())
                                    {
                                        ...
                                        WriteAssertion(writer, isExtension, "expect(e.{0}.{1}.getValue(JulianDate.addSeconds(documentStartDate, 50, new JulianDate()))).toEqualEpsilon(new Cartesian3({2:G15}, {3:G15}, {4:G15}), 1e-13);", propertyName, subProperty.Name, expectedX, expectedY, expectedZ);
                                    }
                                }
                            }
                        }
                    ...
                    using (writer.OpenScope())
                    {
                        ...
                        foreach (var property in schemaProperties)
                        {
                            ...
                            if (property.IsInterpolatable)
                            {
                                ...
                                using (writer.OpenScope())
                                {
                                    var firstValueProperty = properties.First(p => p is { IsValue: true, ValueType.IsArray: true });
                ...
                foreach (var property in schemaProperties)
                {
                    ...
                    using (writer.OpenScope())
                    {
                        ...
                        if (property.IsInterpolatable)
                        {
                            foreach (var valueProperty in properties.Where(p => p is { IsValue: true, ValueType.IsArray: true }).Skip(1))
                            {
                                writer.WriteLine("using (var packet = m_writer.OpenPacket(m_output))");
                                using (writer.OpenScope())
                                {
                ...
                using (writer.OpenScope())
                {
                    if (schema.Properties.FirstOrDefault(p => p.Name == "properties") is { ValueType.AdditionalProperties: { } additionalProperties } propertiesProperty)
                    {
                        writer.WriteLine("using (var packet = m_writer.OpenPacket(m_output))");
                        using (writer.OpenScope())
                        {
                            const string id = "sampled_custom";
                            writer.WriteLine("packet.WriteId(\"{0}\");", id);
                            WriteAssertionBoth(writer, "expect(e = dataSource.entities.getById('{0}')).toBeDefined();", id);
                            ...
                            using (writer.OpenScope())
                            {
                                foreach (var valueProperty in additionalProperties.ValueType.Properties.Where(p => p is { IsValue: true, ValueType.IsArray: true }))
                                {
                                    string propName = $"custom_{valueProperty.Name}";
                                    writer.WriteLine("using (var w2 = w.Open{0}Property(\"{1}\"))", additionalProperties.ValueType.NameWithPascalCase, propName);
                                    using (writer.OpenScope())
    ...
    private static string GetSubPropertyName(string propertyName, Property subProperty)
    {
        ...
        if (subPropertyName == "gltf")
            subPropertyName = "uri";
        else if (propertyName == "polygon" && subPropertyName is "positions" or "holes")
            subPropertyName = "hierarchy";
        return subPropertyName;
    }

    private static string GetPropertyName(Property property, out bool isExtension)
    {
        string propertyName = property.Name;
        string? extensionPrefix = property.ValueType.ExtensionPrefix;
        switch (extensionPrefix)
        {
            case { Length: > 0 }:
                isExtension = true;
                return propertyName[(extensionPrefix.Length + 1)..];
            default:
                isExtension = false;
                return propertyName;
        }
    ...
    private static void GetUniqueValue(string propertyName, string valueName, Property valueProperty, Property parentProperty,
                                       out string value, out string valueSuffix,
                                       out string assertionValue, out string? assertionEpsilon,
                                       out string valueType)
    {
        value = valueSuffix = assertionValue = valueType = "";
        assertionEpsilon = null;

        byte[] HashData() => MD5.HashData(Encoding.UTF8.GetBytes(valueName + parentProperty.Name + valueProperty.Name));

        var hash = new Lazy<byte[]>(HashData);

        int GetNumber(int n) => BitConverter.ToUInt16(hash.Value, n % 8);
        ...
        switch (valueTypeName)
        {
            ...
            case "UnitCartesian3":
            {
                ...
                value = $"new UnitCartesian({x:G15}, {y:G15}, {z:G15})";
                assertionValue = $"new Cartesian3({x:G15}, {y:G15}, {z:G15})";
            ...
            case "UnitCartesian3List":
            {
                ...
                value = $"CreateList(new UnitCartesian({x1:G15}, {y1:G15}, {z1:G15}), new UnitCartesian({x2:G15}, {y2:G15}, {z2:G15}))";
                string prefix = "";
                string suffix = "";
                if (parentProperty.ValueType.Name == "DirectionList")
                {
                ...
                assertionValue = $"[ {prefix}new Cartesian3({x1:G15}, {y1:G15}, {z1:G15}){suffix}, {prefix}new Cartesian3({x2:G15}, {y2:G15}, {z2:G15}){suffix} ]";
                assertionEpsilon = "1e-14";
            ...
            case "CartographicRadians":
            {
                ...
                value = $"new Cartographic({longitude:G15}, {latitude:G15}, {height:G15})";
                string units = isDegrees ? "Degrees" : "Radians";
                assertionValue = $"Cartesian3.from{units}({longitude:G15}, {latitude:G15}, {height:G15})";
            ...
            case "CartographicRadiansList":
            {
                ...
                value = $"CreateList(new Cartographic({longitude1:G15}, {latitude1:G15}, {height1:G15}), new Cartographic({longitude2:G15}, {latitude2:G15}, {height2:G15}))";
                if (propertyName == "polygon.hierarchy")
                {
                    valueSuffix = ".positions";
                }
                string units = isDegrees ? "Degrees" : "Radians";
                assertionValue = $"[ Cartesian3.from{units}({longitude1:G15}, {latitude1:G15}, {height1:G15}), Cartesian3.from{units}({longitude2:G15}, {latitude2:G15}, {height2:G15}) ]";
                valueType = "List<Cartesian>";
                return;
            }
            ...
            case "CartographicRadiansListOfLists":
            {
                ...
                {
                    return $"new Cartographic({values[valuesPerCartographic * index + 0]:G15}, {values[valuesPerCartographic * index + 1]:G15}, {values[valuesPerCartographic * index + 2]:G15})";
                }
                ...
                {
                    return $"Cartesian3.from{units}({values[valuesPerCartographic * index + 0]:G15}, {values[valuesPerCartographic * index + 1]:G15}, {values[valuesPerCartographic * index + 2]:G15})";
                }
            ...
            case "UnitQuaternion":
            {
                ...
                y /= magnitude;
                z /= magnitude;
                value = $"new UnitQuaternion({w:G15}, {x:G15}, {y:G15}, {z:G15})";
                assertionValue = $"new Quaternion({x:G15}, {y:G15}, {z:G15}, {w:G15})";
                assertionEpsilon = "1e-14";
            ...
            case "Rgbaf":
            {
                ...
                switch (valueTypeName)
                {
                    case "Rgba":
                        ...
                        break;
                    case "Rgbaf":
                        assertionValue = $"new Color({r / 255.0:G15}, {g / 255.0:G15}, {b / 255.0:G15}, {a / 255.0:G15})";
                        assertionEpsilon = "1e-14";
                        break;
                }
            ...
            case "CartographicRectangleRadians":
            {
                ...
                value = $"new CartographicExtent({w:G15}, {s:G15}, {e:G15}, {n:G15})";
                assertionValue = $"{(isDegrees ? "Rectangle.fromDegrees" : "new Rectangle")}({w:G15}, {s:G15}, {e:G15}, {n:G15})";
    ...
    private static void WriteAssertion(CodeWriter writer, bool isExtension, [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string assertion, params object[] args)
    {
    ...
    private static void WriteAssertionBoth(CodeWriter writer, [StringSyntax(StringSyntaxAttribute.CompositeFormat)] string assertion, params object[] args)
    {
    ...
    private static string AssertionIndent => new(' ', AssertionIndentAmount * 4);
    private static int AssertionIndentAmount { get; set; } = 0;
}
