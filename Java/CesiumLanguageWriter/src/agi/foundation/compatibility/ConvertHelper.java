...
public final class ConvertHelper {
	 ...
	 * as an offset in the input array, and the number of elements in the array to
	 * convert.
	 * 
	 * @param inArray
	...
	public static String toBase64String(byte[] inArray, int offset, int length, Base64FormattingOptions options) {
		...
		int opts = Base64.NO_OPTIONS;
		if (options.contains(Base64FormattingOptions.INSERT_LINE_BREAKS))
			opts |= Base64.DO_BREAK_LINES;

		try {
			return Base64.encodeBytes(inArray, offset, length, opts);
		} catch (IOException e) {
