...
public class LeapSeconds {
	...
	/**
	 *  
	Convert <code>date</code> from TAI to UTC, if possible.
	...
	public final JulianDate convertTaiToUtc(JulianDate date) {
		...
		boolean temp_0 = tryConvertTaiToUtc(date, out$result_1);
	...
	/**
	 *  
	Try to convert <code>date</code> from TAI to UTC, if possible.
	...
	{@link JulianDate}, if it was possible to convert.
	 * @return <see langword="true" /> if <code>date</code> could be converted
	...
	final private JulianDate getDateForIndex(int index) {
		if (index < 0) {
	...
	final private double getOffsetForIndex(int index) {
		if (index < 0) {
			return 10.0;
		}
	...
	static private class CompareLeapSecondDate implements Comparator<LeapSecond> {
