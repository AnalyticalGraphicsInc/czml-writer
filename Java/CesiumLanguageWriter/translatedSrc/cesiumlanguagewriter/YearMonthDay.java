 ...
 */
public class YearMonthDay implements Comparable<YearMonthDay>, IEquatable<YearMonthDay>, ImmutableValueType {
	/**
	...
	Thrown when the <code>year</code>, <code>month</code>, or 
	<code>day</code> is outside of its acceptable range.
	 */
	...
	So, if <code>date</code> is more than 12 hours past the start of the
	...
	number.
	 */
	static private int getAdjustedJulianDayNumber(JulianDate date) {
		int day = date.getDay();
	 ...
	 * @param year The year.
	 * @return <see langword="true" /> if <code>year</code> is a leap year and 
	...
	<code>month</code>)
	 * @return <see langword="true" /> if the representation is valid and 
	<see langword="false" /> if it is not.
	 */
