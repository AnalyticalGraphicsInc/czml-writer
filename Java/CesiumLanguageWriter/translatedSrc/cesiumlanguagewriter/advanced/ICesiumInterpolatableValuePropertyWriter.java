...
public interface ICesiumInterpolatableValuePropertyWriter<TValue> extends ICesiumValuePropertyWriter<TValue>, ICesiumInterpolationInformationWriter {
	...
	to determine the property value at a given time.  The <code>dates</code> need not all
	 ...
	 * @param dates The dates of the samples.  This collection must have the same number of elements as the <code>values</code> collection.
	 * @param values The values corresponding to the <code>dates</code>.  This collection must have the same number of elements as the <code>dates</code> collection.
