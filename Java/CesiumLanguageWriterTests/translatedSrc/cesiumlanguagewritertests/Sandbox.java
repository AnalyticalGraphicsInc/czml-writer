...
public class Sandbox {
    ...
    public final void runExample() {
        String outputPath = PathHelper.combine(TestContext.getCurrentContext().getTestDirectory(), "Sandbox.czml");
        try (Using<OutputStreamWriter> using$0 = new Using<OutputStreamWriter>(StreamWriterHelper.create(outputPath))) {
            ...
            final CesiumOutputStream tempObj$0 = new CesiumOutputStream(streamWriter);
            ...
            try (Using<PacketCesiumWriter> using$2 = new Using<PacketCesiumWriter>(writer.openPacket(output))) {
                ...
                try (Using<BillboardCesiumWriter> using$3 = new Using<BillboardCesiumWriter>(packet.openBillboardProperty())) {
                    final BillboardCesiumWriter billboard = using$3.resource;
                ...
                try (Using<ModelCesiumWriter> using$5 = new Using<ModelCesiumWriter>(packet.openModelProperty())) {
                    final ModelCesiumWriter model = using$5.resource;
                    try (Using<ColorCesiumWriter> using$6 = new Using<ColorCesiumWriter>(model.openColorProperty())) {
                        final ColorCesiumWriter color = using$6.resource;
                        try (Using<CesiumIntervalListWriter<ColorCesiumWriter>> using$7 = new Using<CesiumIntervalListWriter<ColorCesiumWriter>>(color.openMultipleIntervals())) {
                            final CesiumIntervalListWriter<ColorCesiumWriter> intervals = using$7.resource;
                            ...
                            try (Using<ColorCesiumWriter> using$9 = new Using<ColorCesiumWriter>(intervals.openInterval(start, stop))) {
                                final ColorCesiumWriter interval = using$9.resource;
